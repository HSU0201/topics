...
$perPage = 10;
// 進行無條件進位的相除操作，計算總頁數
$pageCount = ceil($waitResult / $perPage);
...
if (isset($_GET["search"])) {
  ...
  // 使用 "search" 參數來篩選查詢
  $sql = "SELECT * FROM shopinfo WHERE shop_name LIKE '%$search%' AND shop_valid=0 AND certified=0";
}
// 檢查是否有分頁參數
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header d-flex align-items-center justify-content-between">
              <h6 class="m-0 font-weight-bold text-primary"> <span class="text-info">等待認證</span>商家列表</h6>
              <!-- 按鈕組 -->
            <?php if (!isset($_GET["search"])) : ?>
              <div class="d-flex justify-content-end">
                <div class="btn-group m-2">
                  <a href="certificationtables.php?page=<?= $page ?>&order=1" class="btn btn-outline-info <?php if ($order == 1) echo "active" ?>">
                    id
                    <i class="bi bi-sort-down-alt"></i>
                  </a>
                  <a href="certificationtables.php?page=<?= $page ?>&order=2" class="btn btn-outline-info <?php if ($order == 2) echo "active" ?>">
                    id
                    <i class="bi bi-sort-up"></i>
                  </a>
                </div>
                <div class="btn-group m-2">
                  <a href="certificationtables.php?page=<?= $page ?>&order=3" class="btn btn-outline-info <?php if ($order == 3) echo "active" ?>">
                  最新修改時間
                    <i class="bi bi-sort-down-alt"></i>
                  </a>
                  <a href="certificationtables.php?page=<?= $page ?>&order=4" class="btn btn-outline-info <?php if ($order == 4) echo "active" ?>">
                  最後修改時間
                    <i class="bi bi-sort-up"></i>
                  </a>
                </div>
                    ...
                    <?php foreach ($rows as $row) : ?>
                      <tr>
                        ...
                        <td class="align-middle text-center">
                          ...
                          <!-- 刪除按鈕，觸發刪除確認視窗 -->
                          <a class="btn btn-warning mx-1" title="認證未通過" type="button" data-toggle="modal" data-target="#exampleModalLong<?= $row["shop_id"] ?>"><i class="bi bi-dash-circle"></i></a>
                        </td>
                      </tr>
                      <!-- 刪除彈出視窗 -->
                        ...
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title" id="deletetable">認證未通過</h5>
