        ...
        <main>
          ...
          <form class="row p-5 need-validation" id="coupon-info" method="post" action="doUpdateCoupon.php" novalidate>
            <div class="mb-3 col-12 d-none">
              ...
              <input type="text" class="form-control" id="coupon-id" name="coupon-id" value="<?= $rows['coupon_id']?>">
            </div>
            <div class="mb-3 col-12">
              <label for="coupon-name" class="form-label">優惠券名稱</label>
              <select class="custom-select" id="coupon-name" name="coupon-name" value="<?=$rows['coupon_name']?>" required>
                <option value="">請選擇</option>
                <option value="滿額折價券" <?php if($rows['coupon_name']=="滿額折價券"){echo 'selected';}?>>滿額折價券</option>
                <option value="滿額折抵券" <?php if($rows['coupon_name']=="滿額折抵券"){echo 'selected';}?>>滿額折抵券</option>
                <option value="特定商品優惠券" <?php if($rows['coupon_name']=="特定商品優惠券"){echo 'selected';}?>>特定商品優惠券</option>
              </select>
            ...
            <div class="mb-3 col-12">
              ...
              <input type="text" class="form-control" id="coupon-intro" name="coupon-intro" value="<?=$rows['coupon_introduce']?>" required>
              <div class="invalid-feedback">請介紹您的優惠券</div>
            </div>
            <div class="mb-3 col-4">
              <label for="coupon-threshold" class="form-label">優惠券門檻</label>
              <input type="number" class="form-control" id="coupon-threshold" name="coupon-threshold"value="<?=$rows['coupon_threshold']?>" required>
              <div class="invalid-feedback">請設定優惠券門檻</div>
            </div>
            <div class="mb-3 col-4">
              <label for="coupon-discount" class="form-label">優惠方式</label>
              <input type="number" class="form-control" id="coupon-discount" name="coupon-discount" value="<?=$rows['coupon_discount']?>" placeholder="打折(0.9表示9折) / 折價 (直接輸入數字)" required>
              <div class="invalid-feedback">請填寫優惠方式</div>
            </div>
            <div class="mb-3 col-4">
              <label for="coupon-code" class="form-label">優惠碼</label>
              <input type="text" class="form-control" id="coupon-code" name="coupon-code" value="<?=$rows['coupon_code']?>" placeholder="6~10碼大寫英、數字" required>
            ...
            <div class="mb-3 col-8">
              ...
              <div class="row">
                <div class="col">
                  <input type="date" class="form-control" id="coupon-start" name="coupon-start" value="<?=$rows['coupon_start']?>" required>
                  <div class="invalid-feedback">請選擇開始日期</div>
                </div>
                <p class="fs-3 m-2"> ~ </p>
                <div class="col">
                  <input type="date" class="form-control" id="coupon-exp" name="coupon-exp" value="<?=$rows['coupon_exp']?>" required>
            ...
            <div class="mb-3 col-4">
              ...
              <input type="text" class="form-control" id="coupon-max-count" name="coupon-max-count" value="<?=$rows['coupon_max_count']?>" required>
              <div class="invalid-feedback">請填寫優惠券發送數量</div>
            </div>
            <div class="col d-flex justify-content-end">
              <div class="text-danger fw-500"><?php if(isset($_SESSION['error']['message'])){echo $_SESSION['error']['message'];unset($_SESSION['error']['message']);}?></div>
              <button type="submit" id="create-coupon-btn" class="btn btn-business mx-2">修改</button>
              <a href="<?= $_SESSION['currentUrl'] ?>" class="btn btn-secondary">取消</a>
            </div>
          </form>
        </main>
      </div>
