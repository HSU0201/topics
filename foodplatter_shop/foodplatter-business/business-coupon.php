            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-footer">
                  <button type="button" class="btn btn-business" data-dismiss="modal">取消</button>
          ...
          <div class="row">
            <div class="col d-flex justify-content-between align-items-center p-0">
              <div class="screen-and-sort d-flex mx-5">
                <div class="dropdown">
                  <button class="btn btn-business dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <?php 
                    if(!isset($status)):
                      echo '優惠券狀態';
                    else:
                  ...
                  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="business-coupon.php<?php if(isset($category)){echo "?category=$category";}?>">不篩選狀態</a>
                    <a class="dropdown-item" href="business-coupon.php?status=1<?php if(isset($_GET['category'])){echo "&category=$category";}?><?php if(isset($state_static)){echo "&state=$state_static";}?>">未上架</a>
                    <a class="dropdown-item" href="business-coupon.php?status=2<?php if(isset($_GET['category'])){echo "&category=$category";}?><?php if(isset($state_static)){echo "&state=$state_static";}?>">已上架</a>
                    <a class="dropdown-item" href="business-coupon.php?status=3<?php if(isset($_GET['category'])){echo "&category=$category";}?><?php if(isset($state_static)){echo "&state=$state_static";}?>">已過期</a>
                  </div>
                </div>
                <div class="dropdown">
                  <button class="btn btn-business dropdown-toggle mx-2" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                  <?php 
                    if(!isset($category)):
                      echo '優惠券類型';
                    else:
                  ...
                  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="business-coupon.php<?php if(isset($status)){echo "?status=$status";}?>">不篩選類型</a>
                    <a class="dropdown-item" href="business-coupon.php?<?php if(isset($_GET['status'])){echo "status=$status";}?>&category=1<?php if(isset($state_static)){echo "&state=$state_static";}?>">滿額折價券</a>
                    <a class="dropdown-item" href="business-coupon.php?<?php if(isset($_GET['status'])){echo "status=$status";}?>&category=2<?php if(isset($state_static)){echo "&state=$state_static";}?>">滿額折抵券</a>
                    <a class="dropdown-item" href="business-coupon.php?<?php if(isset($_GET['status'])){echo "status=$status";}?>&category=3<?php if(isset($state_static)){echo "&state=$state_static";}?>">特定商品優惠券</a>
                  </div>
                </div>
                <a href="business-coupon-create.php" class="btn btn-business"><i class="bi bi-plus"></i></a>
              </div>
              <nav class="col d-flex justify-content-end mx-5 p-0" aria-label="Page navigation example">
                <ul class="pagination m-0">
                  <li class="page-item">
                    <form action="business-coupon.php?<?php if(isset($status)){echo "status=$status";}?><?php if(isset($category)){echo "&category=$category";}?><?php if(isset($state_static)){echo "&state=$state_static";}?>" method="post">
            ...
            <table class="table table-hover text-nowrap mx-5 my-3">
              <thead>
                <tr class="text-center">
                  ...
                  <th scope="col" class="align-middle">說明</th>
                  <th scope="col" class="align-middle">門檻</th>
                  <th scope="col" class="align-middle">折扣</th>
                  <th scope="col" class="align-middle">優惠卷代碼</th>
                  <th scope="col" class="align-middle">有效期限</th>
                  <th scope="col" class="align-middle">發送數量</th>
                  <th scope="col" class="align-middle">最後修改日期<a class="btn btn-business btn-sm" href="business-coupon.php?<?php if(isset($status)){echo "status=$status";}?><?php if(isset($category)){echo "&category=$category";}?>&state=<?=$state?>"><i class="bi bi-caret-down-fill"></i></a></th>
                  <th scope="col" class="align-middle">修改/刪除</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr class="<?php if($today>$row['coupon_exp']){echo 'text-danger';}?>" title="已上架優惠券">
                  ...
                  <td class="d-flex justify-content-around">
                    ...
                    <form action="business-coupon-update.php" method="post" class="d-inline-block">
                      ...
                      <button type="submit" class="btn btn-business" title="修改"><i class="bi bi-pencil-square"></i></button>
                    </form>
