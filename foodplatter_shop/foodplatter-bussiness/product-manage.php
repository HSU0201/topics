...
// 計算存在的食物總筆數
$sqlTotal = "SELECT * FROM food_list WHERE food_valid=1 AND foodCategory_id=21";
$resultTotal = $conn->query($sqlTotal);
$totalProduct = $resultTotal->num_rows;

// 食物分類
$sqlCategory = "SELECT * FROM food_category WHERE food_valid=1 AND foodCategory_id=21";
...
if (isset($_GET["search"])) {
    ...
    WHERE food_list.food_name LIKE '%$search%' AND food_list.food_valid=1 AND food_list.foodCategory_id=21"; // 加上 AND valid=1，避免被刪除後還搜尋的到
    ...
    $sql = "SELECT food_list.*,food_category.name FROM food_list
    JOIN food_category ON food_list.category_id = food_category.id
    WHERE food_list.food_valid=1 AND food_list.foodCategory_id=21 ORDER BY $orderSql LIMIT $startItem, $perPage";
} else if(isset($_GET["category"])){
    $category = $_GET["category"];
    $sql = "SELECT food_list.*,food_category.name FROM food_list 
    JOIN food_category ON food_list.category_id = food_category.id 
    WHERE food_list.category_id = $category
    AND food_list.food_valid=1 AND food_list.foodCategory_id=21 
    ...
    $sql = "SELECT food_list.*,food_category.name FROM food_list 
    JOIN food_category ON food_list.category_id = food_category.id 
    WHERE food_list.food_valid=1 AND food_list.foodCategory_id=21 
    ORDER BY food_list.food_id ASC LIMIT 0,$perPage";
    $order = 1; // 預設為升冪排列
}
// 執行搜尋動作結束
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">商品管理列表</h1>
                    ...
                    <div class="d-sm-flex align-items-center justify-content-end mb-4">
                        <a class="btn btn-danger text-white btn-no-border shadow-sm" href="./delete-product-manage.php" title="已刪除的商品 ">已刪除的商品 <i class="bi bi-trash3"></i></a>
                    </div>
                    ...
                    <div class="container-fluid">
                        <div class="py-2 d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                if (isset($_GET["search"])) : ?>
                                    <!-- 增加一個返回的按鈕 -->
                                    <a class="btn btn-info text-white" href="/foodplatter/foodplatter-bussiness/product-manage.php" title="返回全部商品列表">
